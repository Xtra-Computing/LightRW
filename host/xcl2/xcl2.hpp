...
class Memory {
   public:
    static decltype(&xclGetMemObjDeviceAddress) getMemObjDeviceAddress;
    static void init(const cl_platform_id& platform) {
        void* bar = clGetExtensionFunctionAddressForPlatform(platform, "xclGetMemObjDeviceAddress");
        getMemObjDeviceAddress = (decltype(&xclGetMemObjDeviceAddress))bar;
    }
};
}
