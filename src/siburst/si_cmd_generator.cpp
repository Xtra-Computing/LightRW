...
extern "C" {
    void si_cmd_generator( vertex_descriptor_ordered_stream_t  &input,
...
#pragma HLS PIPELINE style=frp
                    ...
                    SEND_DYNAMIC_CMD(DYNAMIC_BURST_MAX_LENGTH, burst_cmd)
                    //}
                    //else {
            ...
            {
                //command for invalid data
                ap_uint<1> stream_last = 0;
                ap_uint<1> end_of_burst = 0;
                ap_uint<16> dynamic_id = ((dynamic_burst_id & 0x7fff));
                ap_uint<16> burst_id_with_end = burst_id & 0x7fff;
                burst_id_with_end.range(15, 15) = end_of_burst;
                burst_cmd_gen(0, 0, 0, 0, id, burst_cmd, burst_id_with_end, dynamic_id, 0,1);
