...
#define SEND_DYNAMIC_CMD(N,S) {                                                         \
    ...
    burst_cmd_gen(last,burst_length,mask_start,mask_end,                                \
                 id,S,burst_id_with_end,dynamic_id,stream_last,state);                  \
}


extern "C" {
    void cmd_generator( vertex_descriptor_ordered_stream_t  &input,
                        burst_cmd_stream_t                  &burst_32_cmd,
                        burst_cmd_stream_t                  &burst_1_cmd )
    {
...
cmd_main: while (1)
        {
            ...
            uint16_t state       = vd.ap_member(vertex_descriptor_ordered_t, state);
