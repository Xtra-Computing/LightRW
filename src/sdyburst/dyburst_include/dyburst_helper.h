#ifndef __DYBURST_HELPER_H__
#define __DYBURST_HELPER_H__
...
#define BURST_INIT()    burst_cmd_item_t data = pkg.data;                           \
                        ...
                        uint16_t state = data.ap_member(burst_cmd_t, state);        \
...
#pragma HLS INLINE
    ...
    bcmd.ap_member(burst_cmd_t, state)      = state;
    pkg.data = bcmd;
    pkg.last = last;
    burst_cmd.write(pkg);
}
