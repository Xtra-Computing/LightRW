...
int main(int argc, char** argv) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_store = cl::Kernel(program, "test_store_512:{test_store_512_1}", &err));
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({   buffer_vertex_desp, buffer_res
                                                    }, 0 /*0 means from host*/));
